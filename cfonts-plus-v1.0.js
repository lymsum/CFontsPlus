...
/**
 * 用法示例：
 * node cfonts-cli.js "Hello Utralkill lymsum" -f block -c cyan,magenta -l 2 --line-height 1.5 -a center -o lymsum.png
 * ver: 1.0.0
 * author: stone lee
...
// import minimist from 'minimist';  // 这里对带值的args[++i]坑做了手动处理，没有使用minimist库
...
class Cli {
  ...
  static argsProcessing() {
    ...
    let isFirst = false; //是否找到第一个startsWith('-')的参数，后面找到"-"开头的参数不加进texts中
    ...
    console.log(`✅ ---arg:--- text: ${texts} textArray: ${textArray}`);

    // 简单参数解析器, optionseq是第一个-开头的点
    for (let i = optionseq; i < args.length; i++) {
      ...
      if (!arg.startsWith('-') && !isFirst) {  //&& !text
        ...
        //   options.lineHeight = parseFloat(args[++i]);
          ...
          console.log(`---args-extra.singleFile1:--- ${extra.singleFile}`);
    ...
    console.log(`---args-extra.singleFile2:--- ${extra.singleFile}`);

    // const background = extra.background || 'transparent';

    console.log(`----texts----: ${texts} ${JSON.stringify(options)} ${JSON.stringify(extra)} ${outputFile}`);
...
class Utils {
  ...
  static render(text,options,extra) {
    ...
    const lines = rendered.array.map(Utils.parseAnsi24bit);
    ...
    const actualWidth = maxChars * charWidth * scaleSize;
    ...
    console.log(`---render---: text.length: ${text.length} lines: ${textRenderLineCounts} maxchars: ${maxChars} width: ${actualWidth}: height: ${actualHeight}`);
    ...
    // 使用实际的宽和高值
    result.width = actualWidth; //点(.)操作符，适用于大多数场景
    result.height = actualHeight;
    result.startX = extra.padding;
    result.startY = (textRenderLineCounts*charHeight-charHeight)/2 - extra.padding;
    result.charHeight = charHeight;
   ...
   */
  static typewriter(text, delay = 20) {
    let i = 0;
    function printNext() {
      if (i < text.length) {
...
class CanvasManager {
  ...
  initContext(ctx, background, width, height, fontSize, scaleSize, padding, align='top') {
    ...
    const cur_fontSize = fontSize * scaleSize;
    ...
    // font和textBaseline设置
    ctx.font = `bold ${cur_fontSize}pt monospace`; //等宽字体，也可用px sans-serif`;
...
function saveCFontsHDColorStretchImage(texts, options, extra, outputFile, termWidth, stretchFactor, shadowLines) {
  ...
  const oklines = Utils.whitespaceTrim(rendered.string.split('\n'), termWidth);
  console.log(`✅ oklines: ${oklines}`);
  // 拉伸
  const lines = Utils.stretchRender(oklines, stretchFactor, shadowLines);
  console.log(`lines: ${lines.join(' ')}`);
  // 高清解析每一行的颜色段
  const parsedLines = lines.map(Utils.parseAnsi24bit);   // [{text, color}]
...
function saveCFontsHDColorLine(texts, options, extra, outputFile, termWidth, stretchFactor, shadowLines) {
  ...
  //   // 2. 高清、字符居中、左右空白字符去除、拉伸渲染等处理
  //   // 去掉左右多余空白并居中,这里只取render后的string,不含颜色值之类，并要作换行处理
  //   const trimLine = Utils.whitespaceTrim(rendered.string.split('\n'), termWidth);
  //   // 拉伸
  //   const stretchLines = Utils.stretchRender(trimLine, stretchFactor, shadowLines);
  ...
  //   // 高清解析每一行的颜色段
  //   const parsedLines = stretchLines.map(Utils.parseAnsi24bit);  //[{text, color}]
...
function textHDColorPlain(text, options, termWidth, stretchFactor, shadowLines) {
    ...
    // 去掉左右多余空白并居中,这里只取render后的string,不含颜色值之类，并要作换行处理
    const trimLine = Utils.whitespaceTrim(rendered.string.split('\n'), termWidth); //lymsum换行
    // const trimLine = Utils.whitespaceTrim(rendered.array, termWidth);
 ...
 * @description 
 */
function textSaveFile(text, options, extra, outputFile, termWidth, stretchFactor, shadowLines) {
  let tmpRender = [];
  const singleFile = extra.singleFile || 0;
  console.log(`textRender-singleFile: ${singleFile}`);
  console.log(`textRender-termWidth: ${termWidth}`);
  ...
  if (singleFile == 0) {
    ...
    renderSaveFile(text, options,extra,outputFile);
  }
  else if (singleFile == 1) {
    for(const tmptext of text.split(' ')) {
      ...
      const lastDotIndex = outputFile.lastIndexOf('.');
      const newOutputFile = outputFile.substring(0,lastDotIndex) + `_${tmptext}` + outputFile.substring(lastDotIndex);
      renderSaveFile(tmptext, options,extra,newOutputFile);
    }
  }
  else {
    // tmpRender = textHDColorPlain(text, options, termWidth, stretchFactor, shadowLines);
    ...
    renderSaveMultibleLinesFile(text, options, extra, outputFile, termWidth, stretchFactor, shadowLines);
  }
}
 ...
 */
function renderSaveFile(texts, options, extra, outputFile) {
  ...
  // CFonts.render扩展
  const rendered = Utils.render(texts, options, extra);
  ...
  showWelcome();
  // Utils.typewriter(rendered.string, 1);
  // Utils.typewriter(lines.join('\n'), 1);
 ...
 * @description 把多个render数据，输出到多行文本的单个文件
 */
function renderSaveMultibleLinesFile(text, options, extra, outputFile, termWidth, stretchFactor, shadowLines) {
  // text为"lymsum Utralkill"的原始文件
  // 定义font宽和高与字体大小比例
  const FONTWIDTH_RATIO = 0.80;
  const FONTHEIGHT_RATIO = 1.2; //计算结果为5/3，但不知准不准
  ...
  // for(const key in render)  {
  for(const tmptext of text.split(' ')) {
    let rendered = Utils.render(tmptext, options, extra);
    const lines = rendered.render;
    const width = rendered.width;
    const height = rendered.height;
...
function main() {
    try {
        ...
        textSaveFile(texts, options, extra, outputFile, termWidth, stretchFactor, shadowLines);

        // testRender(texts, options, extra, outputFile);
        // renderSaveFile(texts, options, extra, outputFile);
...
// ESM 入口判断,仅当直接运行该文件时才执行 main()
// if (require.main === module) {//CommonJS写法
// if (import.meta.url === `file://${process.argv[1]}`) { //windows反斜杠问题
